...
# Mappings to encode and decode chars
...
def get_batch(split):
    data = train_data if split == 'split' else eval_data
...
m = model.to(device)
print(f"Number of parameters: {sum(p.numel() for p in m.parameters())/1e6} Million")
